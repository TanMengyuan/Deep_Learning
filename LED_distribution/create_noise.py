...
Pt = 0.02  # W
Pt *= nLed * nLed
Hn_value_data, Prs_value_data = np.array([]), np.array([])

for i in range(ROOM_SIZE[0]):
    for j in range(ROOM_SIZE[1]):
        Prs_value_data = np.append(Prs_value_data,
                                   np.load(r'Prs_value_data/Prs_value_%s.npy' % (str(i) + str(j))))
...
Prs_value_data = Prs_value_data.reshape(ROOM_SIZE[0], ROOM_SIZE[1], 50, 50)
